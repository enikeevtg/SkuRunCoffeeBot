...
def items_kb(items: List[rq.DrinkItem]) -> InlineKeyboardMarkup:
    ...
    for item in items:
        keyboard.button(text=item.name,
                        callback_data=f'item_{item.id}_{item.name}')
    keyboard.button(text='⬅️ Назад в основное меню',
                    callback_data=back_to_categories_btn_cb)
    return keyboard.adjust(1).as_markup()


def categories_kb_builder(items_kbs: Dict[str, List[InlineKeyboardMarkup]]
...
items_kbs: Dict[str, List[InlineKeyboardMarkup]] = asyncio.run(items_kbs_dict())
