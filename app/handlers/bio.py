...
@router.message(
    F.text == profile_btn_text, StateFilter(None, DrinkOrder.order_done)
)
async def display_user_bio(message: Message, state: FSMContext):
    ...
    await message.answer(
        messages.user_bio.format(message.from_user.id, cur_cup_name),
        reply_markup=bio_kb,
    )
...
@router.message(F.content_type == ContentType.TEXT, NameEdition.set_new_name)
async def set_new_name(message: Message, state: FSMContext):
    ...
    if message.text.replace(" ", "").isalpha() is False:
        ...
        await message.answer(text=messages.set_nickname_done.format(nickname))
